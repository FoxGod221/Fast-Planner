...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  have_target_ = false;
  have_odom_   = false; //示当前是否已经有目标点和里程计数据。

  /*  fsm param  从ROS服务器获取相关参数 */
  ...
  planner_manager_ 是一个 FastPlannerManager 对象的智能指针，它被初始化为一个新的 FastPlannerManager 实例，并调用 initPlanModules 方法进行初始化。
  */
  planner_manager_.reset(new FastPlannerManager);
  planner_manager_->initPlanModules(nh);
  //visualization_ 是一个 PlanningVisualization 对象的智能指针，它被初始化为新的 PlanningVisualization 实例，用于规划过程中的可视化操作。
  visualization_.reset(new PlanningVisualization(nh));

  /* callback */
  //创建一个周期为 10 毫秒（0.01 秒）的定时器，定时触发 KinoReplanFSM 类的 execFSMCallback 回调函数。用于执行状态机的更新。
  exec_timer_   = nh.createTimer(ros::Duration(0.01), &KinoReplanFSM::execFSMCallback, this);
  safety_timer_ = nh.createTimer(ros::Duration(0.05), &KinoReplanFSM::checkCollisionCallback, this);    //可能用于检查碰撞或安全问题。

  waypoint_sub_ =
      nh.subscribe("/waypoint_generator/waypoints", 1, &KinoReplanFSM::waypointCallback, this);  //接收航点数据，并通过回调函数 waypointCallback 进行处理。
  odom_sub_ = nh.subscribe("/odom_world", 1, &KinoReplanFSM::odometryCallback, this); //接收里程计数据，并通过回调函数 odometryCallback 进行处理。
  //发布一些有关重置规划器的信息
...
//状态基，用于处理在机器人运行过程中遇到各种情况下，是选择急停、还是重新规划、还是继续执行等。 规划器的调用也是在这个函数内  这个函数被定时调用
...
bool KinoReplanFSM::callKinodynamicReplan() {
  // //调用规划器 前端加后端所有流程都在这个kinodynamicReplan中实现 在planner_manager.cpp中
  bool plan_success =
      planner_manager_->kinodynamicReplan(start_pt_, start_vel_, start_acc_, end_pt_, end_vel_);   
  //如果规划成功
  if (plan_success) {
    //调用 planner_manager_ 对象的 planYaw 方法，传入 start_yaw_（起始航向角）。计算航向角的轨迹。
    planner_manager_->planYaw(start_yaw_);
    //获取 planner_manager_ 中的本地数据（local_data_），并将其指向 info，这个数据包含了规划过程中的各种轨迹信息。
    ...
    //从 position_traj_ 获取控制点（位置轨迹），并将每个控制点的坐标（x, y, z）转化为 geometry_msgs::Point 类型，存入 bspline.pos_pts 数组。
    ...
    //时间节点向量获取轨迹的结点信息，并将每个结点添加到 bspline.knots 数组中。
    Eigen::VectorXd knots = info->position_traj_.getKnot();
    for (int i = 0; i < knots.rows(); ++i) {
      bspline.knots.push_back(knots(i));
    }
    //从 yaw_traj_ 获取航向角的控制点，并将每个航向角值添加到 bspline.yaw_pts 数组中。
    ...
    //将填充好的 B 样条轨迹（bspline）发布到 /planning/bspline 话题上。
    ...
    cout << "generate new traj fail." << endl;    //如果 plan_success 为 false，则输出错误信息 "generate new traj fail."，并返回 false，表示规划失败。
    return false;
  }
}
